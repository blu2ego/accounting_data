...
for(p in start_a001:end_a001){
  list_a001 <- read.csv(lists_a001[p])
  ...
  for(q in 1:nrow(list_a001)){
    recept_no <- as.character(list_a001[q, "rcept_no"])
    
    request_url_a001 <- paste0("https://opendart.fss.or.kr/api/document.xml?",
                               "&crtfc_key=", key_dart,
                               "&rcept_no=", recept_no)
    
    path_zip <- paste(path_dir, "/", list_a001[q, "year"], "_", recept_no, ".zip", sep = "")
