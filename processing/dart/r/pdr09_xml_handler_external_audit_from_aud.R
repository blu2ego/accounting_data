...
gross_audit_external_from_aud <- list()
...
##### 외부 감사 실시 내용(집합) #####
    ...
    names(indv_audit_external_from_aud)[n_file] <- fiscal_year
  }
...
#### write for rds #####
saveRDS(gross_audit_external_from_aud, file = paste0(main_dir, audit_report_rds_aud, "external_audit_aud.rds"))


##### write for json #####
gross_audit_external_json <- jsonlite::toJSON(gross_audit_external_from_aud, pretty = TRUE, auto_unbox = TRUE, encoding = "UTF-8")
write(gross_audit_external_json, file = paste0(main_dir, audit_report_json_aud, "external_audit_aud.json"))