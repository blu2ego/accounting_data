...
setwd(file.path(mainDir, "results/biz_report/xml/2020-07-25"))
...
for(l in 1:length(biz_reports_xml_list)){
  ...
  if(nrow(corp_biz_report) >= 1){
    ...
    write.csv(corp_biz_report, paste0(mainDir, biz_report_list_csv_Dir, corp_biz_report[1, "corp_code"], ".csv"),
                row.names = FALSE) 
    }
  }
}
