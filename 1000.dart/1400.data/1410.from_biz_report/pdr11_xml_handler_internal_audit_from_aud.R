...
# list up audit report from audit report(F001)
...
gross_audit_internal_from_aud <- list()
indv_audit_internal_from_aud <- list()
...
##### 내부회계관리제도 감사 또는 검토의견(집합) #####
    ...
    indv_audit_internal_from_aud[[n_file]] <- internal_audit_report
    names(indv_audit_internal_from_aud)[n_file] <- fiscal_year
  }
    
  gross_audit_internal_from_aud[[n_corp]] <- indv_audit_internal_from_aud
  names(gross_audit_internal_from_aud)[n_corp] <- corp_code
  indv_audit_internal_from_aud <- list()
  closeAllConnections()
}
...
#### write for rds #####
saveRDS(gross_audit_internal_from_aud, file = paste0(main_dir, audit_report_rds_aud, "internal_audit_aud.rds"))


##### write for json #####
gross_audit_internal_json <- jsonlite::toJSON(gross_audit_internal_from_aud, pretty = TRUE, auto_unbox = TRUE, encoding = "UTF-8")
write(gross_audit_internal_json, file = paste0(main_dir, audit_report_json_aud, "internal_audit_aud.json"))